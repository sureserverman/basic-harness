...
New companion plugin: `appdev-ip-advisor`. App-development IP issue-spotter modelled on `fintech-legal-advisor` — same architecture (one Opus-pinned subagent, one skill, one setup-Routine command, vault-citizen via vault-companion), different practice area (intellectual property for app developers rather than fintech regulation).

### Added
- **`appdev-ip-advisor` plugin (v0.1.0)**. New companion plugin covering app-development IP issue-spotting across six jurisdictions (US federal + key state right-of-publicity, EU, UK, Russia, Singapore, UAE). Regimes: trademark / brand naming, trade dress and visual-style mimicry (with explicit treatment of physical-product UI mimicry — film-camera dials, rangefinder controls, console-controller bezels, watchfaces — surfacing the unregistered trade dress + design-patent + copyright stack that attaches), copyright (icons, fonts, music, sound effects, photography, code, AI-generated content), open-source licence compliance (GPL/AGPL/LGPL/MIT/Apache/MPL/EPL plus source-available non-OSS BSL/SSPL/Elastic License), right of publicity / likeness / voice cloning (with explicit state-by-state US treatment of California §3344, New York §50/51, Tennessee "Elvis Act", Texas, Florida; plus EU GDPR Art. 9 biometric overlay), App Store Review Guideline 5.2 and Google Play Impersonation / IP / Deceptive Behavior, EU AI Act Art. 50 (in force 2026-08-02). Components:
  - **`appdev-ip-analyst`** (Opus-pinned subagent) — careful IP tier. Hard rules: jurisdiction + asset type + distribution channel first, issue lists not verdicts, citation grounding mandatory, no software-patent analysis (refuses and routes to registered patent attorney), no trademark freedom-to-use opinions, no case-law interpretation (names cases for counsel only), no DMCA-strategy advice, no drafting of final-form regulated content (final App Store metadata / EULAs / Acknowledgements / AI-content disclosure language), take-to-counsel block always present.
  - **`appdev-ip-triage`** (skill) — Phase -1 (Claude for Legal install nudge, one-shot) → Phase 0 (jurisdiction + asset type + distribution channel — mandatory hard gate) → Phase 0.5 (vault recall via `vault-companion-recall`) → Phase 1 (match cell to issue regime cluster) → Phase 2 (walk issue checklist) → Phase 3 (structured issue list with one block per issue: regime / cite, question for counsel, confidence) → Phase 4 (save via `vault-companion-append` to `<vault>/Legal/`, with `[[Lanham Act §43(a)]]` / `[[App Store Review Guideline 5.2]]` / `[[GPL-3.0]]` / `[[AI Act Art. 50]]` / `[[Cal. Civ. Code §3344]]` wikilink enrichment) → Phase 5 (mandatory take-to-counsel block).
  - **`/appdev-ip-advisor:setup-ip-triage-routine`** (slash command) — wires `appdev-ip-triage` into a Cowork Routine on the user's asset / EULA / licence-bundle / draft inbound folder. Step 0 mandatory privacy gate (Routine cloud posture; pre-launch / NDA assets refused into the Routine). Step 0.5 one-shot Claude for Legal install nudge. Step 1 source selection (Drive / Box / iManage / NetDocuments / Docusign). Step 2 jurisdiction + distribution channel + asset-type mix capture. Step 3 optional Microsoft Word tracked-change output for `.docx` inputs. Step 4 Routine prompt block. Step 5 Cowork UI walkthrough. Step 6 setup-history save to `~/.claude/appdev-ip-advisor.local.md`. Step 7 verification on first triggered run. Step 8 hand-off.
  - **`docs/routines/ip-triage-on-drive-routine.md`** — copy-paste Routine template for hand-wiring without the slash command. Includes the same privacy header as Step 0 of the setup command.
- **Soft dependency on `intellectual-property`** (Claude for Legal practice-area plugin) declared in `plugin.json`. The root marketplace's `allowCrossMarketplaceDependenciesOn: ["claude-for-legal"]` already covers this — no further whitelist edit required.
- **Vault-citizen integration.** `appdev-ip-triage` reuses the existing `Legal/` category in the personal vault schema — sits side-by-side with `fintech-legal-advisor`'s triages, distinguished by frontmatter `type: ip-triage` (vs. `legal-triage` for fintech) and by wikilink surface (IP statutes / app-store policy / OSS-licence names vs. financial-regulation names). No schema change required.

### Changed
- **Marketplace 0.7.0 → 0.8.0.** Top-level description names the app-development IP issue-spotter alongside the fintech one. New eighth plugin entry. Persona triplet expanded from "personal users / fintech operators" to "personal users / fintech operators / app developers" in README + marketplace + golden-path walkthrough.
- **README golden path** (personal user / fintech operator / app developer) expanded with the appdev-ip-advisor install step, the setup-ip-triage-routine wiring step, the interactive appdev-ip-triage skill, and the appdev-ip-analyst subagent — mirroring how fintech-legal-advisor appears in the same walkthrough.

### Rationale
The user spec was: "add one more plugin, similar to the fintech one, but for app development and copyright questions — like using brand names or elements of styles of other products. Plugin should utilize Claude for Legal." App-development IP is a natural sibling to fintech regulation: same audience cohort (founders / operators who need fast issue-spotting before they ship and before they call counsel), same architectural pattern (Cowork Routine for high-volume non-sensitive flow + interactive skill for sensitive one-offs, Opus-pinned analyst, vault-citizen save, take-to-counsel block as a hard guarantee), but distinct enough in coverage that a generic IP plugin (Claude for Legal's own `intellectual-property` plugin) doesn't ship the app-dev-specific checklists — App Store Review Guideline 5.2, OSS-licence contagion at static-link time, sound-effect chain-of-title, AI Act Art. 50 disclosure architecture, in-app Acknowledgements-screen attribution surfaces, voice-cloning right-of-publicity stacks. User asked specifically that the plugin also handle physical-product UI mimicry (e.g., "film camera controls images utilized in the mobile app") — covered explicitly in the trade-dress section of both the skill and the analyst. User confirmed software patents should be out of scope: the plugin refuses and routes to a registered patent attorney rather than improvising patent-adjacent analysis.
